...
final class WPOMP
{
    ...
    public function __construct()
    {
        ...
        add_action('init', [$this, 'loadLanguages'], 10);

        add_filter('plugin_action_links_' . plugin_basename(__FILE__), [$this, 'pluginLinkPage'], 10, 1);
        add_filter('plugin_row_meta', [$this, 'pluginRowMeta'], 10, 2);

        add_action('admin_enqueue_scripts', [$this, 'assets'], 10, 1);

        add_action('admin_menu', [$this, 'addMenu'], 10);
        add_action('admin_init', [$this, 'registerSections'], 10);
        add_action('admin_init', [$this, 'registerFields'], 10);
        add_action('admin_init', [$this, 'addFields'], 10);
    }
    ...
    public static function activation()
    {
        ...
        if (
            isset($upload_path) && empty($upload_path) && isset($upload_url_path) && empty($upload_url_path)
        ) {
    ...
    public function assets($hook)
    {
        ...
        if ($hook == 'settings_page_' . self::SLUG . '-options') {
            wp_enqueue_script(
                self::SLUG,
                plugins_url('assets/' . self::SLUG . '.js', __FILE__),
                ['jquery'],
                self::VERSION,
                true,
            );
        ...
        if ($hook == 'options-media.php' || $hook == 'settings_page_' . self::SLUG . '-options') {
            wp_enqueue_style(
                self::SLUG,
                plugins_url('assets/' . self::SLUG . '.css', __FILE__),
                null,
                self::VERSION,
                'all',
            );
    ...
    public function addFields()
    {
        $fields = [
            ...
            'upload_url_path' => [
                ...
                'description' => sprintf(
                    __(
                        'Simply specify the url for your upload folder. Be careful, if you want a domain other than %s, make sure to point the domain (DNS) to the desired folder on your current server. The plugin can not upload to any other server than this one.',
                        self::I18N,
                    ),
                    '<strong>' . home_url() . '</strong>',
                ),
                'placeholder' => 'https://',
            ],
        ];
...
add_action('plugins_loaded', ['RVOLA\WPOMP', 'load'], 10);
register_activation_hook(__FILE__, ['RVOLA\WPOMP', 'activation']);
